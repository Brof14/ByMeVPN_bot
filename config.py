...
INBOUND_ID: int = int(os.getenv("INBOUND_ID", "5"))

# Log 3x-UI config at import time so any misconfiguration is visible in logs
import logging as _logging
_logging.getLogger(__name__).info(
    "3x-UI config → host=%s user=%s inbound_id=%d",
    os.getenv("XUI_HOST", ""), os.getenv("XUI_USERNAME", ""), INBOUND_ID
)
...
# YooKassa webhook server settings

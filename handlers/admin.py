...
from marzban_client import delete_marzban_user
...
async def cb_admin_del_key(callback: CallbackQuery, bot: Bot):
    ...
    if k and k.get("uuid"):
        deleted_from_panel = await delete_marzban_user(user_id)
...
async def cb_del_user(callback: CallbackQuery, bot: Bot):
    ...
    # Delete from panel
    for uuid in uuids:
        await delete_marzban_user(uuid)
    await callback.message.edit_text(
...
async def cb_cleanup_do(callback: CallbackQuery, bot: Bot):
    ...
    for key in expired_keys:
        try:
            success = await delete_marzban_user(key["user_id"])
            if success:
                cleaned_count += 1
...
async def cb_system_status(callback: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        connected, _ = await test_marzban_connection()

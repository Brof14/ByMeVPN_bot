...
from marzban_client import delete_marzban_user, get_marzban_user, format_traffic
...
async def cb_key_info(callback: CallbackQuery, bot: Bot):
    ...
    try:
        user_id = callback.from_user.id
        marzban_user = await get_marzban_user(user_id)
        if marzban_user:
...
async def cb_key_delete_confirm(callback: CallbackQuery, bot: Bot):
    ...
    ok = await delete_marzban_user(user_id)
    if not ok:
        logger.warning(

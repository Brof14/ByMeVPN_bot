...
from database import (
    ...
    has_used_trial, try_claim_trial,
...
async def cmd_start(message: Message, bot: Bot, state: FSMContext):
    ...
    # Delete the /start command message itself first, then clean history.
    # Deleting anchor first prevents iOS from briefly showing the "Start" button.
    try:
        await bot.delete_message(message.chat.id, message.message_id)
    except Exception:
        pass
    # Clean the previous 50 messages (anchor already deleted, so use anchor-1)
...
async def receive_config_name(message: Message, bot: Bot, state: FSMContext):
    ...
    limit_ip = data.get("limit_ip", 1)   # default 1 if not set (safety fallback)
    ...
    yk_payment_id = data.get("_yk_payment_id")  # set only for YooKassa payments

    # Debug logging to see what data we received
    logger.info("receive_config_name DEBUG: user=%d data=%s", user_id, data)
    logger.info("Config name: user=%d name='%s' days=%d limit_ip=%d method=%s", user_id, name_input, days, limit_ip, method)

    success = await deliver_key(
        bot=bot, user_id=user_id, chat_id=message.chat.id,
        config_name=name_input, days=days, limit_ip=limit_ip,
    ...
    if not success and trial_uid:
        from database import reset_trial
        try:
            await reset_trial(trial_uid)
        except Exception as e:

...
async def process_referral_click(referrer_id: int, referred_id: int, user_agent: str = None, ip_address: str = None) -> bool:
    ...
    try:
        ...
        # Add referral event for the click with 5 days bonus
        ...
        logger.info(f"Referral click processed: referrer={referrer_id}, referred={referred_id}, days_awarded=5")
        return True
...
async def process_payment_referral_bonus(user_id: int, amount: int, bot) -> bool:
    ...
    Awards 50₽ to the referrer for the first payment.
    """
    try:
        from database import get_referrer, ensure_referral_balance, add_referral_earning, get_referral_events
        ...
        bonus_amount = 50
        await ensure_referral_balance(referrer_id)
        await add_referral_earning(referrer_id, user_id, bonus_amount)
        ...
        logger.info(f"Payment bonus awarded: referrer={referrer_id}, referred={user_id}, amount={bonus_amount}")
        return True
    except Exception as e:

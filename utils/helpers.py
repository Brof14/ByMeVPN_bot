...
async def update_menu(
    ...
    try:
        ...
        # Truncate text if it's too long for photo caption
        if photo and len(text) > TELEGRAM_CAPTION_LIMIT:
            original_text = text
            text = text[:TELEGRAM_CAPTION_LIMIT - 50] + "\n\n<i>...текст обрезан. Для полной инструкции выберите другой формат.</i>"
            logger.warning(f"Text truncated from {len(original_text)} to {len(text)} characters for photo caption")
...
def read_legal_file(filename: str) -> str:
    """
    Read legal document file from data/legal directory
    ...
    try:
        ...
        legal_path = current_dir / "data" / "legal" / filename
        
        if not legal_path.exists():
            logger.error(f"Legal file not found: {legal_path}")
            return "Файл не найден"
        
        with open(legal_path, 'r', encoding='utf-8') as file:
            content = file.read()
            
        logger.info(f"Legal file read successfully: {filename}")
        return content
        
    except Exception as e:
        logger.error(f"Error reading legal file {filename}: {e}")
        return f"Ошибка чтения файла: {str(e)}"


def get_legal_document_content(document_type: str) -> str:
    """
    Get content of legal document by type
    
    Args:
        document_type: Type of document ('offer', 'privacy', 'recurrent')
        
    Returns:
        Document content as string
    """
    document_files = {
        "offer": "Договор_публичной_оферты_ByMeVPN.txt",
        "privacy": "Политика_конфиденциальности_ByMeVPN.txt",
        "recurrent": "Соглашение_о_рекуррентных_платежах_ByMeVPN.txt"
    }
    
    filename = document_files.get(document_type)
    if not filename:
        logger.error(f"Unknown document type: {document_type}")
        return "Неизвестный тип документа"
    
    return read_legal_file(filename)
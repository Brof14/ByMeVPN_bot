...
async def create_client(email: str, days: int) -> Optional[str]:
    """
    ...
    async def _attempt():
        ...
        logger.info("Client created: email=%s uuid=%s days=%d", email, client_id, days)
        return client_id

    try:
        return await _with_retry(_attempt)
    except Exception as e:
        logger.error("create_client permanently failed for '%s': %s", email, e)
        return None
